...
class SearchResult:
    ...
    source_field: str  # 'title', 'description', 'tags', 'personal_notes', 'raw_data', 'chunks'
    relevance_score: float  # Combined score: (field_weight × 0.7) + (vector_similarity × 0.3)
    field_weight: float  # Weight based on field reliability (0.6-1.0)
    similarity_distance: Optional[float] = None  # Vector embedding distance (lower = more similar)
...
class ComprehensiveSearchEngine:
    ...
    def comprehensive_search(
        ...
        try:
            ...
            logger.info(f"[COMPREHENSIVE_SEARCH] Starting multi-field search for query: {query_text[:80]}...")
            ...
            logger.info(f"[COMPREHENSIVE_SEARCH] Retrieved {len(chunk_results.get('documents', [[]])[0])} initial results")
            ...
            if chunk_results["documents"] and len(chunk_results["documents"]) > 0:
                for i, doc in enumerate(chunk_results["documents"][0]):
                    ...
                    distance = chunk_results["distances"][0][i] if chunk_results["distances"] else None
                    ...
                    vector_similarity = self._distance_to_similarity(distance) if distance is not None else 0.8
            ...
            logger.info("[COMPREHENSIVE_SEARCH] Searching summaries collection for overview...")
            ...
            logger.info(f"[COMPREHENSIVE_SEARCH] Returning {len(final_results)} results")
    ...
    def search_by_field(
        ...
        try:
            ...
            logger.info(f"[SEARCH_BY_FIELD] Found {len(field_results)} results in {field_name}")
        ...
        try:
            ...
            for field_name in self.FIELD_WEIGHTS.keys():
                try:
                    ...
                    logger.warning(f"Could not get stats for field {field_name}: {str(e)}")
                    stats[field_name] = {"count": 0, "weight": self.FIELD_WEIGHTS[field_name]}
