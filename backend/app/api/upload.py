...
async def upload_file(
    ...
    try:
        if not supabase:
            return JSONResponse(
                status_code=500, 
                content={"error": "Server configuration error: Missing Supabase credentials"}
            )
        ...
        if file_size > max_size:
            return JSONResponse(
                status_code=400, 
                content={"error": f"File size exceeds {max_size / (1024 * 1024)}MB limit"}
            )
        ...
        public_url_data = supabase.storage.from_(bucket_name).get_public_url(unique_file_name)
