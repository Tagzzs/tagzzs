...
async def queue_youtube_extraction(
    body: QueueRequest, user: Dict[str, Any] = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to queue extraction: {str(e)}"
        )
...
async def get_extraction_result(
    ...
    try:
        ...
        if status == "completed":
            ...
            if result_res.data:
                response["error"] = result_res.data[0].get(
...
async def list_youtube_queue(user: Dict[str, Any] = Depends(get_current_user)):
    """
        ...
        try:
            ...
            if old_items.data:
                ...
                if old_ids:
                    ...
                    supabase.table("extraction_results").delete().in_(
                        "queue_id", old_ids
                    ).execute()

                    # 3. Delete from queue
                    supabase.table("extraction_queue").delete().in_(
                        "id", old_ids
                    ).execute()
