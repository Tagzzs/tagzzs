...
async def update_tag_counts_on_content_change(
    user_id: str, old_tag_id: Optional[str] = None, new_tag_id: Optional[str] = None
) -> None:
    """
...
async def update_tag_counts_on_array_change(
    user_id: str, old_tag_ids: List[str] = [], new_tag_ids: List[str] = []
) -> None:
    """
...
async def recalculate_all_tag_counts(user_id: str) -> None:
    ...
    try:
        ...
        tags_collection = (
            admin_db.collection("users").document(user_id).collection("tags")
        )
