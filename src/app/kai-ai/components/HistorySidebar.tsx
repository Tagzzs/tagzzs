  ...
  return (
    <aside
      ...
      className={`history-sidebar-transition h-full flex flex-col bg-[#0a0a0a] shrink-0 z-30 overflow-hidden
             ${
               isHistoryOpen
                 ? "w-64 border-r border-[#27272a]"
                 : "w-0 history-collapsed border-none"
             }`}
    >
      <div className="p-4 pt-5 shrink-0 min-w-[16rem]">
        ...
        >
          ...
          <span className="sidebar-text text-sm font-medium text-zinc-300 group-hover:text-white">
            New Chat
          </span>
        </div>
      </div>
      <div className="flex-1 overflow-y-auto px-3 pb-4 min-w-[16rem]">
        <div className="mb-6">
          <h3 className="sidebar-text px-3 mb-2 text-[10px] font-bold text-zinc-600 uppercase tracking-widest">
            Recent
          </h3>
            ...
            {isLoading ? (
              ...
              chatList.map((chat) => (
                ...
                >
                  <Brain
                    weight="regular"
                    className="shrink-0 group-hover:text-[#D8CEF0]"
                  />
                  <span className="sidebar-text truncate text-xs flex-1">
                    {chat.title}
                  </span>
                  <div
                    role="button"
                    className="shrink-0 opacity-0 group-hover:opacity-100 p-1 hover:bg-zinc-800 rounded transition-all z-10"
