...
interface GraphNode {
    ...
    data?: { name: string; desc: string; image: string; content: string; contentId?: string };
}
...
export default function NeuralGraphPage() {
    ...
    const { messages: chatMessages, sendMessage, isSending: isChatLoading } = useChat();
    ...
    const selectNode = (node: GraphNode) => {
        ...
        if (node.type === 'category') {
            ...
            if (catIdx !== -1) {
                ...
                setExpandedGroups(new Set([`cat-${catIdx}`, ...(subIdx !== -1 ? [`sub-${catIdx}-${subIdx}`] : [])]));
            }
        }
    };
    ...
    const handleChatSubmit = async (text: string, source: 'floating' | 'sidebar') => {
        if (!text.trim()) return;
        ...
        if (source === 'floating') {
            ...
            const foundNode = nodesRef.current.find(n => n.label?.toLowerCase() === searchTerm);
    ...
    const renderMiniGraph = (centerNode: GraphNode) => {
        ...
        if (centerNode.type === 'root') {
            ...
            const catIndex = deepData.findIndex((c: { name: string }) => c.name === catName);
        ...
        centerText.textContent = centerNode.label ? centerNode.label.substring(0, 4).toUpperCase() : 'NODE';
    ...
    useEffect(() => {
        ...
        const project = (x: number, y: number, z: number) => {
            ...
            return { x: width / 2 + x1 * perspective * zoomRef.current, y: height / 2 + y2 * perspective * zoomRef.current, scale: perspective * zoomRef.current, z: z2 };
        };

        // Separate Update and Render logic
        const update = () => {
            // Lerp focus and rotation
            focusPosRef.current.x += (targetFocusPosRef.current.x - focusPosRef.current.x) * 0.1;
            focusPosRef.current.y += (targetFocusPosRef.current.y - focusPosRef.current.y) * 0.1;
            focusPosRef.current.z += (targetFocusPosRef.current.z - focusPosRef.current.z) * 0.1;
        ...
        const render = () => {
            ...
            linksRef.current.forEach(link => {
                ...
                if (s && t && s.pz < cameraRef.current.z) {
                    ...
                    ctx.strokeStyle = `rgba(167, 139, 250, ${isLeaf ? 0.05 : 0.15 * s.scale})`;
                    ctx.stroke();
                }
            });
            ...
            projected.forEach(p => {
                ...
                if (p.type === 'dust') {
                    ...
                    if (p.type === 'root' || p.type === 'category') {
                        ...
                        if (selectedNodeRef.current && (selectedNodeRef.current.id === p.id || selectedNodeRef.current.id === p.parent)) showLabel = true;
                    ...
                    if (showLabel && p.label) {
                        ...
                        ctx.font = `${p.type === 'category' ? 600 : 400} ${Math.max(9, fontSize * p.scale)}px Inter`;
        ...
        const createContentGraph = () => {
            ...
            if (graphData.nodes.length > 0) {
                const nodeList: GraphNode[] = graphData.nodes.map(n => ({
                    ...
                    data: n.data ? {
                        ...
                        image: n.data.thumbnailUrl || 'https://picsum.photos/seed/default/800/400',
                ...
                for (let i = 0; i < 200; i++) {
                    ...
                    nodeList.push({
                        ...
                        z: r * Math.cos(phi),
                        radius: Math.random() * 1.5, color: COLORS.dust
                    });
                }
            ...
            const nodeList: GraphNode[] = [
                { id: 'root', label: 'Root', type: 'root', x: 0, y: 0, z: 0, radius: 12, color: COLORS.root }
            ];
            const linkList: Link[] = [];
        ...
        const handleMouseDown = (e: MouseEvent) => {
            ...
            if (node) {
                ...
                if (selectedNodeRef.current && !detailLayerRef.current?.classList.contains('active')) {
                    resetCamera();
                }
        ...
        const handleMouseMove = (e: MouseEvent) => {
            if (isDraggingRef.current) {
                targetRotationRef.current.y += (e.clientX - lastMouseRef.current.x) * 0.005;
                targetRotationRef.current.x += (e.clientY - lastMouseRef.current.y) * 0.005;
                lastMouseRef.current = { x: e.clientX, y: e.clientY };
                return;
            }
            const rect = canvas.getBoundingClientRect();
