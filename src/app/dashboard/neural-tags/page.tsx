...
export default function TagsPage() {
  ...
  const handleUpdateTag = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/user-database/tags/edit`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDeleteTag = async (tag: TagItem) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/user-database/tags/delete`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleViewItems = (tag: TagItem) => {
    ...
    router.push(
      `/dashboard/memory-space?tags=${encodeURIComponent(tag.tagName)}`
    );
  };
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              ...
              >
                ...
                <DialogContent className="bg-white dark:bg-gray-900 rounded-2xl border-[#E6E6FA] max-w-md">
                  <DialogHeader>
                    <DialogTitle className="text-lg text-[#1F2937] dark:text-white">
                      Create New Tag
                    </DialogTitle>
                    <DialogDescription className="text-sm text-[#6B7280]">
                      Add a new tag to organize your content better.
                    </DialogDescription>
                  </DialogHeader>
                  <div className="space-y-3">
                    <div>
                      <Label
                        htmlFor="tag-name"
                        className="text-sm text-[#1F2937] dark:text-white"
                      >
                        Tag Name
                      </Label>
                      <Input
          ...
          {isLoading && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-5">
              {[...Array(6)].map((_, index) => (
                <Card
                  key={index}
                  className="rounded-2xl border-[#E6E6FA] shadow-sm"
                >
          ...
          {error && !isLoading && (
            <Card className="rounded-2xl border-[#E6E6FA] shadow-lg">
              <CardContent className="p-6 sm:p-8 text-center">
                ...
                <h3 className="text-base sm:text-lg font-semibold text-[#1F2937] mb-1.5">
                  Failed to load tags
                </h3>
                ...
                <div className="flex flex-col sm:flex-row justify-center gap-2 sm:gap-3">
                  <Button
                    onClick={() => fetchTags()}
                    variant="outline"
                    className="rounded-xl border-[#E6E6FA] text-xs sm:text-sm h-9"
                  >
                    Try Again
                  </Button>
                  <Button
                    onClick={() => setIsCreateDialogOpen(true)}
                    className="bg-gradient-to-r from-[#A855F7] to-[#7C3AED] text-white rounded-xl text-xs sm:text-sm h-9"
                  >
                    Create First Tag
                  </Button>
                </div>
          ...
          {!isLoading && !error && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-5">
              {filteredTags.map((tag) => (
                <Card
                  key={tag.id}
                  className="rounded-2xl p-3 sm:p-4 space-y-3 bg-white hover:shadow-lg transition-shadow duration-300 border-[#E6E6FA]"
                >
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-7 w-7 p-0 text-[#C8B8E8] hover:text-[#7C3AED] hover:bg-[#F5F3FF] flex-shrink-0"
                          disabled={!!isDeleting}
                        >
                      ...
                      <DropdownMenuContent
                        align="end"
                        className="rounded-xl border-[#E6E6FA]"
                      >
                        <DropdownMenuItem
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent className="rounded-2xl border-[#E6E6FA] max-w-md">
                            <AlertDialogHeader>
                              <AlertDialogTitle className="text-lg text-[#1F2937]">
                                Delete Tag
                              </AlertDialogTitle>
                              <AlertDialogDescription className="text-sm text-[#6B7280]">
                                Are you sure you want to delete the tag "
                                {tag.tagName}"? This action cannot be undone.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                              <AlertDialogCancel
          ...
          {!isLoading && !error && filteredTags.length === 0 && (
            <div className="text-center py-12">
              ...
              {!searchQuery && (
                <Button
                  onClick={() => setIsCreateDialogOpen(true)}
                  className="bg-gradient-to-r from-[#A855F7] to-[#7C3AED] text-white rounded-xl px-4 py-2 text-sm"
                >
