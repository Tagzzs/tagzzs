...
export default function AddContentPage() {
  ...
  const [thumbnailUploadProgress, setThumbnailUploadProgress] = useState<
    string | null
  >(null);
  ...
  const handleAnalyzeUrl = async () => {
    ...
    try {
      ...
      // Check if result indicates an error (result.error is populated in PipelineResult)
      if (result.error) {
      ...
      if (result.content) {
        ...
        const hasMarkdown =
          /(\*{1,2}|_{1,2}|`|#|\[|\]|\(|\)|>|-|\+|\d+\.)/g.test(description);
        if (hasMarkdown) {
          setShowDescriptionPreview(true);
        }
      } else {
        throw new Error(
          "Failed to extract content from the provided URL - Invalid response structure"
        );
      }
    } catch (error) {
                          ...
                          <CardContent className="p-4">
                            <div className="space-y-3">
                              ...
                              <p className="text-xs text-slate-600">
                                Add a preview image for this content. This is
                              ...
                              {selectedThumbnail && formData.thumbnailUrl ? (
                                ...
                                <div className="border-2 border-dashed border-violet-300/60 rounded-lg p-6 text-center hover:border-violet-400/60 transition-all">
                                  ...
                                  <Button
                                    ...
                                    onClick={() =>
                                      document
                                        .getElementById("thumbnail-upload-url")
                                        ?.click()
                                    }
