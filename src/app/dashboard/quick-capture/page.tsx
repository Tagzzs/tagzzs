...
import { Plus, X, LinkIcon, Upload, Sparkles, Check, Loader2, ImageIcon } from "lucide-react"
            ...
            if (response.ok) {
              ...
              if (data.thumbnailUrl) {
                ...
                const uploadResponse = await fetch('/api/upload/from-url', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ imageUrl: data.thumbnailUrl })
                });

                if (uploadResponse.ok) {
                  const uploadResult = await uploadResponse.json();
                  if (uploadResult.fileUrl) {
                    setFormData(prev => ({ ...prev, thumbnailUrl: uploadResult.fileUrl }));
                  }
                }
              }
            }
        ...
        if (extractedImage) {
          try {
            const uploadResponse = await fetch('/api/upload/from-url', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ imageUrl: extractedImage })
            });
            ...
            if (uploadResponse.ok) {
              ...
              console.warn('Failed to upload thumbnail from URL');
            }
          } catch (e) {
            console.warn('Error uploading thumbnail:', e);
          }
        }
