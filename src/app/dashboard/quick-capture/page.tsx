...
export default function AddContentPage() {
  ...
  // NEW: State for thumbnail generation UX
        ...
        const timer = setTimeout(async () => {
          try {
            ...
            if (response.ok) {
              ...
              const thumbUrl =
                data.meta_data?.thumbnail_url || data.meta_data?.image_url;

              if (thumbUrl) {
                // Upload to our bucket to make it persistent
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Determine thumbnail
      let extractedThumbnailUrl = "";

      // 1. If Image: Use directly
      if (file.type.startsWith("image/")) {
        extractedThumbnailUrl = result.fileUrl;
      }
      // 2. If Video: Generate thumbnail from file
      else if (file.type.startsWith("video/")) {
        setUploadProgress("Generating thumbnail...");
        const videoThumbBlob = await generateVideoThumbnail(file);
        if (videoThumbBlob) {
          extractedThumbnailUrl = await uploadThumbnailBlob(videoThumbBlob);
        }
      }
      // 3. If PDF: Generate thumbnail from first page
      else if (file.type === "application/pdf") {
        setUploadProgress("Generating PDF preview...");
        const pdfThumbBlob = await generatePdfThumbnail(file);
        if (pdfThumbBlob) {
          extractedThumbnailUrl = await uploadThumbnailBlob(pdfThumbBlob);
        }
      }
      ...
      setFormData((prev) => ({
        ...
        thumbnailUrl: extractedThumbnailUrl || prev.thumbnailUrl,
      }));
  ...
  // Helper to upload generated thumbnail blob
  const uploadThumbnailBlob = async (blob: Blob): Promise<string> => {
    try {
      ...
      if (thumbResponse.ok) {
        ...
        return thumbResult.fileUrl;
      }
    } catch (e) {
      console.error("Failed to upload generated thumbnail", e);
    }
    return "";
  };
          ...
          >
                      ...
                      {extractionError && (
                        <div className="mt-2 p-2.5 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md">
                          <div className="text-xs text-red-600 dark:text-red-400">
                            ...
                            <pre className="mt-1.5 whitespace-pre-wrap font-sans text-xs">
                              {extractionError}
                            </pre>
                          </div>
                          ...
                          {extractionError.includes("PDF") && (
                            <div className="mt-1.5 text-xs text-red-500 dark:text-red-400">
                              <p>
                                <strong>PDF troubleshooting:</strong>
                              </p>
                              <ul className="list-disc list-inside mt-1 space-y-0.5">
                                ...
                                <li>
                                  Academic papers might be behind paywalls
                                </li>
                              </ul>
                            </div>
                          )}
                          ...
                          {!extractionError.includes("PDF") && (
                            <div className="mt-1.5 text-xs text-red-500 dark:text-red-400">
                              <p>
                                <strong>Try these URLs for testing:</strong>
                              </p>
                              <ul className="list-disc list-inside mt-1 space-y-0.5">
                        ...
                        formData.url && (
                          <div className="mt-2 p-2.5 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md">
                            <p className="text-xs text-green-600 dark:text-green-400 flex items-center">
                              ...
                              Content extracted successfully! Review the details
                              below.
                            </p>
                          </div>
                        )}
                  ...
                  <TabsContent value="upload" className="space-y-4">
                    <div className="border-2 border-dashed border-violet-300/60 rounded-xl p-10 text-center bg-gradient-to-br from-violet-50/30 to-indigo-50/30 hover:from-violet-50/50 hover:to-indigo-50/50 transition-all duration-300 hover:border-violet-400/60">
                      ...
                      {isUploading ? (
                        ...
                        <>
                          <p className="text-violet-700 mb-4 text-sm">
                          ...
                          <div className="flex-1 space-y-2">
                            <div className="flex gap-2">
                              <Button
                                ...
                                onClick={() =>
                                  document
                                    .getElementById("file-thumbnail-upload")
                                    ?.click()
                                }
                                disabled={isUploading}
                                className="text-xs border-violet-200 text-violet-700 hover:bg-violet-50 transition-colors"
              ...
              <Card className="border border-violet-200/60 shadow-md shadow-violet-100/20 bg-white/80 backdrop-blur-sm hover:shadow-lg hover:shadow-violet-100/30 transition-all duration-300 rounded-lg">
                <CardContent className="p-4 sm:p-5 md:p-6">
                  <div className="space-y-2 sm:space-y-3">
                    <Label
                      htmlFor="title"
                      className="text-xs sm:text-sm text-slate-900 flex items-center gap-2"
                    >
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    ...
                    >
                      ...
                      <SelectContent className="border-violet-200/60 rounded-lg shadow-lg">
                        <SelectItem
                          value="article"
                          className="text-xs sm:text-sm hover:bg-violet-50 focus:bg-violet-50 rounded-md py-2"
                        >
                          Article
                        </SelectItem>
                        <SelectItem
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-2 sm:pt-4">
              <Button
                type="submit"
                disabled={
                  isSubmitting || !formData.title.trim() || !formData.url.trim()
                }
            ...
            {(!formData.title.trim() || !formData.url.trim()) && (
              <div className="text-xs text-slate-600 bg-violet-50/50 p-3 rounded-lg border border-violet-200/60">
                {!formData.title.trim() &&
                  !formData.url.trim() &&
                  "Please enter a title and URL to save content."}
                {!formData.title.trim() &&
                  formData.url.trim() &&
                  "Please enter a title to save content."}
                {formData.title.trim() &&
                  !formData.url.trim() &&
                  "Please enter a URL to save content."}
              </div>
            )}
          </form>
        </div>
