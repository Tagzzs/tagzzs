...
import { Database, Sparkle, MagnifyingGlass, Plus, X, ArrowRight, Spinner } from '@phosphor-icons/react';
                ...
                <div className="float-input-wrapper flex-1 bg-black border border-zinc-800 rounded-full h-14 flex items-center px-2 pl-2 shadow-2xl relative">
                    ...
                    <div className="h-10 bg-zinc-900 rounded-full flex items-center p-1 border border-zinc-800 shrink-0 mr-3">
                        <button
                            ...
                            className={`h-full px-4 rounded-full text-[10px] font-bold tracking-wide shadow-sm flex items-center gap-1 transition-colors ${searchMode === 'DB' ? 'bg-zinc-700 text-white' : 'text-zinc-500 hover:text-white'}`}
                        >
                            <Database weight="fill" /> DB
                        </button>
                        <button
                            onClick={() => onSetSearchMode('AI')}
                            className={`h-full px-4 rounded-full text-[10px] font-bold tracking-wide transition-colors flex items-center gap-1 ${searchMode === 'AI' ? 'bg-zinc-700 text-white shadow-sm' : 'text-zinc-500 hover:text-white'}`}
                        >
                    ...
                    <div className="flex-1 flex items-center h-full mr-2">
                        {isSearching || (searchMode === 'AI' && isSending) ? (
                             <Spinner className="mr-3 text-purple-400 animate-spin" size={18} />
                        ) : searchMode === 'DB' ? (
                            <MagnifyingGlass weight="bold" className="text-zinc-600 mr-3 text-lg" />
                        ) : (
                        ...
                        <input
                            ...
                            placeholder={searchMode === 'DB' ? "Search in Database..." : "Ask Kai AI anything..."}
                        ...
                        />
                        {query && (
                            <button onClick={clearSearch} className="text-zinc-500 hover:text-white p-1">
                                <X weight="bold" />
                            </button>
                        )}
                    </div>
                        ...
                        <div className="absolute top-full left-0 right-0 mt-2 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl max-h-80 overflow-y-auto">
                            {isSearching ? (
                                ...
                                results.map((result) => (
                                    ...
                                    >
                                        <h4 className="text-sm font-semibold text-white truncate">{result.title}</h4>
                                        <p className="text-xs text-zinc-500 line-clamp-1 mt-1">{result.description}</p>
                                    </div>
                                ))
                            )}
                        </div>
