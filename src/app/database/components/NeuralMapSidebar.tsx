                ...
                >
                    ...
                    {messages.map((msg) => (
                        <div key={msg.id} className={`flex gap-3 fade-in ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-xs text-white border ${msg.role === 'user' ? 'bg-indigo-600 border-indigo-500' : 'bg-zinc-900 border-zinc-800'}`}>
                                {msg.role === 'user' ? 'U' : 'K'}
                            </div>
                            <div className={`p-3 rounded-2xl text-xs leading-relaxed shadow-sm max-w-[80%] ${
                                msg.role === 'user' 
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <div className="relative flex items-center bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden shadow-sm focus-within:border-zinc-700 transition-colors">
                            ...
                            <button 
                                ...
                                className={`pr-3 transition-colors shrink-0 ${!inputValue.trim() || isSending ? 'text-zinc-700' : 'text-zinc-500 hover:text-white'}`}
                            >
