    ...
    return (
        ...
        >
            ...
            <div className="flex-1 p-3 flex flex-col">
                <div className="mb-1">
                    <h4 className="text-xs font-semibold text-white leading-tight group-hover:text-zinc-300 transition-colors line-clamp-2">
                        {content.title}
                    </h4>
                </div>
                ...
                <div className="flex flex-wrap gap-1 mb-2 overflow-hidden mt-auto">
                    {tags.slice(0, 2).map((tag) => (
                        <span
                            key={tag.id}
                            className={`px-1.5 py-0.5 rounded text-[8px] font-medium ${getTagColorFromHex(tag.tagColor)}`}
                            style={{ 
                    ...
                    {tags.length === 0 && content.contentType && (
                        <span className="px-1.5 py-0.5 rounded text-[8px] font-medium bg-zinc-500/10 text-zinc-400 border border-zinc-500/20">
                            {content.contentType}
                        </span>
                    )}
                </div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-white/5">
                    <span className="text-[8px] text-zinc-600 font-medium">{formatDate(content.createdAt)}</span>
