...
export default function MonthlyBreakdown({ 
    ...
    const { chartData, legendItems, totalItems, topTag, growthPercent } = useMemo(() => {
        if (!content) {
            return {
                chartData: { labels: [], datasets: [{ data: [], backgroundColor: [], borderWidth: 0 }] },
                legendItems: [],
                totalItems: 0,
                topTag: 'None',
                growthPercent: 0
    ...
    if (content.length === 0) {
        return (
            <div className="glass-panel bg-[#050505] mt-12 p-5 flex flex-col items-center justify-center h-64">
                ...
                <p className="text-zinc-600 text-xs">Add content to see your breakdown</p>
            </div>
        );
    }
                ...
                <div className="flex flex-col ml-15 xl:flex-row items-center gap-6">
                    ...
                    <div className="grid grid-cols-2 ml-4 gap-x-6 gap-y-2 h-auto xl:h-28 w-full sm:w-auto">
                        {legendItems.map((item, index) => (
                            ...
                            >
                                <div
                                    className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${hoveredIndex === index ? 'scale-150 ring-2 ring-white/20' : ''
                                        }`}
                                    style={{ backgroundColor: item.color }}
                                />
                                {item.label}
                                <span className={`ml-auto pl-4 transition-colors duration-300 ${hoveredIndex === index ? 'text-white' : 'text-zinc-500'
                                    }`}>
                                    {item.percentage}
                                </span>
                            </div>
